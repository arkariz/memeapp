...
| T-207 | ~~Remote copy packs: static manifest + WorkManager fetch, hash verify, atomic swap, GC rule (see §7) — **must exist before the day-14 checkpoint**~~ — **done**, live-verified end-to-end. `RoastPackManifest`/`RoastPackStore`/`RoastPackFetchGateway`/`RoastPackSyncWorker` mirror the existing `gif` package's split (pure file logic vs. network vs. WorkManager job); `RoastPackStore.applyDownloadedPack` verifies sha256 + a non-empty `templates` array before atomically flipping the `packs/current` pointer (zip-slip guarded), so a bad manifest/pack can never corrupt the active pack. `RoastEngine`/`RoastOverlayController` resolve against the active synced pack first, falling back to the bundled asset pack on anything from "nothing synced yet" through "somehow unreadable now." `RoastPackSyncWorker` runs daily (WorkManager) plus once immediately on `startWatcher()`, so a fresh grant doesn't wait a full day to pick up a new pack. **Live-verified** with a real manifest+pack hosted on GitHub Releases (repo made public — a private repo's release assets need an auth token the app can't safely embed): sync applied (`pack sync version=1 applied=true`), `files/packs/current` → `1` with `roast_pack.json` + all 8 mascot `.webp` present, a real roast rendered the exact template text from the *downloaded* pack (not the bundled fallback), and the failure path (deliberately wrong sha256) was rejected safely — `sync failed` logged, `current` untouched, no crash. **Not yet done:** the currently-hosted `pack_v1.zip` still ships the old v2 template set (with `{intent}`-quoting templates since removed from the bundled pack, see roast_pack.json's v3 notes) — needs repackaging/re-hosting before it's safe to actually point a real device at that manifest again. `RoastPackSyncWorker.schedule()`'s dedupe-across-repeat-`startWatcher()`-calls (KEEP policy) also still needs a manual check. | M | T-105 | P1 |
