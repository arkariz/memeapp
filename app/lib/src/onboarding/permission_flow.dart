...
class _PermissionFlowState extends State<PermissionFlow> {
  ...
  late final List<PermissionStepConfig> _steps;
  int _index = 0;
  ...
  void initState() {
    ...
    _steps = [
      PermissionStepConfig(
        ...
        checkGranted: () async => (await _api.getStatus()).hasUsageAccess,
        onRequest: _api.openUsageAccessSettings,
      ),
      PermissionStepConfig(
        stepIndex: 2,
        totalSteps: 4,
        ...
        checkGranted: () async => (await _api.getStatus()).hasOverlayPermission,
        onRequest: _api.openOverlayPermissionSettings,
      ),
      PermissionStepConfig(
        stepIndex: 3,
        totalSteps: 4,
        ...
        checkGranted: () async => (await _api.getPermissionSnapshot()).hasNotificationPermission,
        onRequest: _api.requestNotificationPermission,
      ...
      PermissionStepConfig(
        ...
        checkGranted: () async => (await _api.getPermissionSnapshot()).isIgnoringBatteryOptimizations,
        onRequest: _api.requestBatteryExemption,
        allowSkip: true,
      ),
    ];
  }
  ...
  Future<void> _finish() async {
    await _api.setOnboardingComplete();
    await _api.startWatcher();
    if (!mounted) return;
    Navigator.of(context).pushAndRemoveUntil(
      MaterialPageRoute(builder: (_) => const ScaffoldCheckPage()),
      (route) => false,
    );
  }
