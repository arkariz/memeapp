...
import com.arkarizdev.bonked.core.analytics.AnalyticsFlushWorker
...
object WatcherCore {
     ...
     * that promises exactly this scope. Excludes this app itself — Bonked
     * can't watch Bonked.
     */
    ...
    suspend fun getWatchedApps(context: Context): List<WatchedAppConfig> {
        val rows = BonkedDatabase.get(context).watchedAppDao().all()
    ...
    suspend fun saveWatchedApps(context: Context, apps: List<WatchedAppConfig>) {
        ...
        BonkedDatabase.get(context).watchedAppDao().replaceAll(
